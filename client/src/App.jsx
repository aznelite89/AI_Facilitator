...
\nUser Name: Ali
\nOrganization: Bata
\nBio: Experienced manager on shoes Manucfacturing industry
...
export default function App() {
  ...
  const prettyInit = useMemo(
    () => (initResult ? JSON.stringify(initResult, null, 2) : ""),
    [initResult]
  )
  const prettyFac = useMemo(
    () => (facResult ? JSON.stringify(facResult, null, 2) : ""),
    [facResult]
  )
  ...
  async function callInitiate() {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error(
          json?.error ? JSON.stringify(json.error) : "Request failed"
        )
      setInitResult(json)
    } catch (e) {
    ...
    <div className="page">
      <header className="header">
        <div>
          ...
          <p className="sub">
            Paste <code>users_info</code> and <code>conversation</code> as
            strings, then call the two endpoints.
          </p>
        </div>
      ...
      <div className="grid">
        <section className="card">
          ...
          <textarea
            value={usersInfo}
            onChange={(e) => setUsersInfo(e.target.value)}
            rows={14}
          />
      ...
      <footer className="footer">
        <span>
          Server: <code>server/</code> (Express) • Client: <code>client/</code>{" "}
          (Vite + React)
        </span>
      </footer>
    </div>
